...
const navigation = [
  ...
  },
  {
      ...
      roles: ["pd", "pcd", "burser", "auditor"],
      icon: require("../assets/purchase-order.png")
  },
  {
      name: "Sales",
      path: "/sales",
            ...
            <Disclosure.Panel className="hidden md:block">
              <div className="space-y-1 px-2 pb-3 pt-2 sm:px-3">
                {navigation.map((item) =>
                  item.roles.includes(localStorageData.role) ? (
                    <Link to={item.path} key={item.name}>
                      <div className="flex items-center gap-2 rounded-lg hover:bg-gray-100 px-4 py-2 text-gray-700">
                        <img width={20} alt={`${item.name}-icon`} src={item.icon} />
