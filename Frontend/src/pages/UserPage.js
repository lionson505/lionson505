...
const UserPage = () => {
    ...
    const fetchUsers = async () => {
        ...
        setError(null);
        try {
            const response = await fetch('https://netcompassion-backend.onrender.com/api/users');
...
const handleUpdate = async (updatedUser) => {
    setShowModal(false);
    try {
        const response = await fetch(`https://netcompassion-backend.onrender.com/api/users/${updatedUser._id}`, {
            method: 'PUT',
            headers: {
...
const handleAdd = async (newUser) => {
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete) {
            try {
                const response = await fetch(`https://netcompassion-backend.onrender.com/api/users/${id}`, {
                    method: 'DELETE',
                });
