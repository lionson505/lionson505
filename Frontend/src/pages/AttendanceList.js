...
function AttendanceList() {
  ...
  const fetchAttendanceData = async () => {
    ...
    try {
      ...
      // const response = await fetch(`https://netcompassion-backend.onrender.com/api/attendance?userId=${authContext.userId}`);
      const compassionId = authContext.user.compassion;
      console.log(compassionId)
// const response = `https://netcompassion-backend.onrender.com/api/attendance/compassion/${compassionId}`;

      const response = await fetch(`https://netcompassion-backend.onrender.com/api/attendance/compassion/${compassionId}`);
  ...
  return (
    <div className="col-span-12 lg:col-span-10 flex justify-center">
      <div className="flex flex-col gap-5 w-11/12">
        ...
        <div className="px-3">
          {Object.keys(statusCounts).map(date => (
            <div key={date} className="flex justify-between cursor-pointer" onClick={() => handleDateClick(date)}>
              <span>{date}</span>
              <span>Present: {statusCounts[date].present}</span>
              <span>Absent: {statusCounts[date].absent}</span>
              <span>Saved: {statusCounts[date].saved}</span>
            </div>
          ))}
        </div>
