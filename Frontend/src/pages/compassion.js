...
const CompassionPage = () => {
    ...
    const handleUpdateCompassion = async (updatedRecord) => {
        try {
            const response = await fetch(`https://netcompassion-backend.onrender.com/api/compassion/${updatedRecord._id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleDeleteCompassion = async (id) => {
        try {
            await fetch(`https://netcompassion-backend.onrender.com/api/compassion/${id}`, {
                method: 'DELETE',
            });
